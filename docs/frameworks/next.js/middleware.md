...
```js highlight= filename="middleware.js" switcher

  runtime: 'nodejs',
}
```

```ts highlight= filename="middleware.ts" switcher
...
- `NextResponse.next( })` to make `requestHeaders` available upstream
- **NOT** `NextResponse.next()` which makes `requestHeaders` available to clients
...
```js
expect(unstable_doesMiddlewareMatch()).toEqual(false);
```
