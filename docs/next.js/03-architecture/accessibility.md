...
When transitioning between pages rendered on the server (e.g. using the `<!-- a -->` tag) screen readers and other assistive technology announce the page title when the page loads so that users understand that the page has changed.
