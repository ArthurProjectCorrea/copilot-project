...
Confirm that you want to migrate your database and create your initial tables by hitting <!-- kbd -->Y on your keyboard.
...
5. Start the development server again:
   ...
   npm run dev
   ```
6. Open the Nuxt DevTools (by hitting <!-- kbd -->Shift+<!-- kbd -->Option+ <!-- kbd -->D) and click the Prisma logo in the left sidenav to open Prisma Studio. Then add a new `User` record by specifying values for the `name` and `email` fields.
